            ...
            {section === 'teaching' && isInstructor && (
              <div style={{ border: `1px solid ${RULE}`, backgroundColor: '#FFFFFF' }}>
                ...
                {(!teachingData?.courses?.length) ? (
                  ...
                  const activeCourses = teachingData.courses.filter(c => (c.classes || []).some(cls => new Date(cls.class_date) >= now && cls.status !== 'cancelled'));
                  const completedCourses = teachingData.courses.filter(c => !(c.classes || []).some(cls => new Date(cls.class_date) >= now && cls.status !== 'cancelled'));
                  const renderCourse = (course, { isCompleted } = {}) => {
                      ...
                      const today = now;
                      const futureClasses = classes.filter(c => new Date(c.class_date) >= today && c.status !== 'cancelled');
                      const pastClasses = classes.filter(c => new Date(c.class_date) < today || c.status === 'cancelled');
