...
(async () => {
  try {
    ...
    // Filter for Eliana Baey
    const elianaOrders = allOrders.filter(order => {
      ...
      return (firstName.includes('eliana') && lastName.includes('baey')) ||
             email.includes('ebjm2000');
    });

    console.log('Total orders for Eliana Baey:', elianaOrders.length);
    console.log('');

    elianaOrders.forEach((order, idx) => {
