...
function generate({ dayOfWeek, startDate, endDate, timeSlot, holidayExclusions, specialNotes }) {
  const subject = `VES Course: Wheelthrowing Beginner/Ext 6 Weeks - 3x Course Package — ${dayOfWeek}s, ${startDate} - ${endDate} (${timeSlot})`;
  ...
  const body = `
    <p style="margin: 0 0 20px; font-size: 15px; line-height: 1.6; color: #282828;">
      Great news! Your <strong>3-Course &middot; 6 Wks Wheelthrowing Package</strong> is confirmed!
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #F9EDE6; border-radius: 8px; margin: 0 0 24px;">
      <tr>
        <td style="padding: 16px 20px;">
          ...
          <p style="margin: 0 0 2px; font-size: 14px; color: #282828;">3 consecutive 6-week courses (18 weeks total)</p>
          ${holidayLine}
        </td>
      </tr>
    </table>
