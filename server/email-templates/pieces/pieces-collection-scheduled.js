...
function generate({ studentName, pieceCount, collectionDate, appUrl }) {
  ...
  const body = `
    ...
    <p style="margin: 0 0 16px; font-size: 16px; line-height: 1.6; color: #333;">
      ...
      <strong>${count} piece${plural ? 's' : ''}</strong>.
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin: 0 0 24px;">
      <tr>
        <td align="center">
          <a href="${escUrl(appUrl)}/gallery" style="display: inline-block; padding: 14px 32px; background-color: #C4622D; color: #ffffff; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 15px;">
            View My Pieces
          </a>
        </td>
      </tr>
