...
function generate({ studentName, pieceCount, photoUrl, appUrl, collectionDate }) {
  ...
  const body = `
    ...
    <p style="margin: 0 0 16px; font-size: 16px; line-height: 1.6; color: #333;">
      Your <strong>${count} piece${count !== 1 ? 's' : ''}</strong>
      ${count !== 1 ? 'are' : 'is'} out and ready —
      ${count !== 1 ? 'they are' : 'it is'} in the glass cabinet outside the studio.
    </p>
    <p style="margin: 0 0 16px; font-size: 16px; line-height: 1.6; color: #333;">
      ${pretty
        ? `See you on <strong>${pretty}</strong>. The cabinet is outside, so come by whenever suits you that day.`
        : `The cabinet is outside the studio, so come pick ${count !== 1 ? 'them' : 'it'} up anytime.`}
    </p>
    ${safePhoto ? `
    ...
    <p style="margin: 0 0 20px; font-size: 16px; line-height: 1.6; color: #333;">
      Once you've collected ${count !== 1 ? 'them' : 'it'}, just tap the button below to let us know.
    </p>
