...
async function enrollShinInTuesdayWT() {
  try {
    console.log('\n🔍 Finding Shin N Chan...\n');

    // Get Shin N Chan's customer record
    const { data: joey, error: joeyError } = await supabaseDb.supabase
      .from('customers')
      .select('*')
      .eq('email', 'shine2starry@gmail.com')
      .single();

    if (joeyError || !joey) {
      console.error('❌ Shin N Chan not found:', joeyError);
      return;
    }

    console.log('✅ Found Shin N Chan (ID:', joey.id, ')');
    ...
    console.log(`\n📚 Enrolling Shin in: ${courseIdentifier}\n`);
    ...
    console.log(`\n✨ Successfully enrolled Shin in ${enrolled}/${courseClasses.length} weeks!`);
    ...
    if (updateError) {
      ...
      console.log(`✅ Updated Shin's classes_used to ${enrolled}`);
    }

    console.log('\n🎉 Shin N Chan is now enrolled in the Tuesday WT course!\n');
