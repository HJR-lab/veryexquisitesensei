...
app.get('/api/admin/students/list', authenticateToken, requireAdmin, asyncHandler(async (req, res) => {
  ...
  // Placeholder students from a multi-spot order: the purchaser was emailed one
  // tokenised form and has not filled it in yet, so we hold a name like
  // "Asyiqin Rashaid (2)" and an autogenerated +dup address. Nothing about that
  // student is confirmed, so the Users list must not badge them as an ordinary
  // active student. status='pending' on the request IS the fact.
  const { data: pendingDetailRequests } = await supabaseDb.supabase
    .from('student_detail_requests')
    .select('placeholder_customer_id')
    .eq('status', 'pending');
  const awaitingDetailsByCustomer = {};
  (pendingDetailRequests || []).forEach(r => {
    if (r.placeholder_customer_id) awaitingDetailsByCustomer[r.placeholder_customer_id] = true;
  });

  // Build attended count per enrollment (attended/completed + past booked)
  const todayStr = new Date().toISOString().split('T')[0];
  const dormantBeforeStr = new Date(Date.now() - 30 * 86400000).toISOString().split('T')[0];
  ...
  for (const enr of allEnrollments) {
    ...
    const entry = {
      ...
      // "Nothing left to come." Either the course window is demonstrably over,
      // or the student has never booked anything and the enrollment has gone
      // quiet — Fusun Can bought 8 handbuilding credits in Feb and booked none
      // of them, and HB rows carry no course_end_date to catch that. Brand-new
      // signups are excluded by the 30-day grace so a fresh purchase is not
      // scolded for classes nobody has had a chance to book yet.
      courseEnded: !hasFutureByEnrollment[enr.id] && Boolean(
        lastClassByEnrollment[enr.id]
        || enr.course_end_date
        || (!committedByEnrollment[enr.id]
            && enr.created_at
            && enr.created_at.split(/[T ]/)[0] < dormantBeforeStr)
      ),
      awaitingDetails: Boolean(awaitingDetailsByCustomer[student.id]),
